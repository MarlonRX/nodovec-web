    ...
    >
      <div className="flex items-center gap-4">
        <div className="shrink-0">
          <div className="w-12 h-12 md:w-14 md:h-14 rounded-none flex items-center justify-center"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-semibold text-base md:text-lg text-(--text-primary) truncate">{f.name}</h3>
            {f.generate_transactions && (
              <span className="shrink-0 flex items-center gap-1 px-2 py-0.5 rounded-none"
                style={{ backgroundColor: 'rgba(168,85,247,0.15)', color: '#a855f7' }}>
                <Zap className="w-3 h-3" />
                <span className="text-xs font-semibold">{t("financing.autoBadge")}</span>
              </span>
            )}
          </div>
        ...
        <div className="hidden sm:flex items-center gap-6 shrink-0">
          <div className="text-right">
            <p className="text-xs font-semibold text-(--text-tertiary) mb-0.5">{t("financing.amount")}</p>
            <p className="font-semibold text-lg text-(--text-primary)">${formatMoney(f.principal_amount)}</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-semibold text-(--text-tertiary) mb-0.5">{t("financing.rate")}</p>
            <p className="font-semibold text-lg" style={{ color: 'var(--accent-primary)' }}>{formatRate(f.annual_interest_rate)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button type="button" onClick={(e) => { e.stopPropagation(); onEdit(f); }}
            className="p-2.5 rounded-none transition-transform hover:scale-105 group"
          ...
          <button type="button" onClick={(e) => { e.stopPropagation(); onDelete(f); }}
            className="p-2.5 rounded-none transition-transform hover:scale-105"
      ...
      <div className="flex sm:hidden items-center justify-between mt-3 pt-3 border-t" style={{ borderColor: 'var(--border-primary)' }}>
        <div>
          <p className="text-xs font-semibold text-(--text-tertiary)">{t("financing.amount")}</p>
          <p className="font-semibold text-base text-(--text-primary)">${formatMoney(f.principal_amount)}</p>
        </div>
        <div className="text-right">
          <p className="text-xs font-semibold text-(--text-tertiary)">{t("financing.rate")}</p>
          <p className="font-semibold text-base" style={{ color: 'var(--accent-primary)' }}>{formatRate(f.annual_interest_rate)}</p>
        </div>
        <div className="text-right">
          <p className="text-xs font-semibold text-(--text-tertiary)">{t("financing.installment")}</p>
          <p className="font-semibold text-base text-(--text-primary)">{f.current_installment}/{f.installments}</p>
        </div>
        {nextPayment && (
          <div className="text-right">
            <p className="text-xs font-semibold text-(--text-tertiary)">{t("financing.nextPayment")}</p>
