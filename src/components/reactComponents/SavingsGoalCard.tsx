...
interface GoalHeaderProps {
  goal: SavingsGoal;
  IconComponent: React.ComponentType<{ size: number; style?: React.CSSProperties }> | undefined;
  t: (key: string) => string;
}
  ...
  <div className="flex items-center gap-4 mb-5">
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2">
        <h3 className="text-base font-bold break-words leading-tight" style={{ color: 'var(--text-primary)' }}>{goal.name}</h3>
...
const GoalProgress: React.FC<GoalProgressProps> = ({ goal, formatCurrency, formatDeadline, t }) => (
  <>
    <div className="flex justify-center mb-4">
      <AnimatedProgressCircle percentage={goal.progress_percentage} size={120} strokeWidth={10} color={goal.color} />
    </div>
    <div className="text-center mb-4">
      <p className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{formatCurrency(goal.current_amount)}</p>
      <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{t('savingsGoals.of') || 'of'} {formatCurrency(goal.target_amount)}</p>
      <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>
        {goal.formatted_progress.remaining} {t('savingsGoals.remaining') || 'remaining'}{goal.progress_percentage >= 100 ? ' ✓' : ''}
      </p>
    </div>
    <div className="flex items-center justify-between mb-4">
      {goal.deadline && (
        <div className="flex items-center gap-1.5" style={{ color: 'var(--text-secondary)' }}>
          <Calendar size={12} />
          <span className="text-xs">{formatDeadline(goal.deadline)}</span>
        </div>
      )}
...
const SavingsGoalCard: React.FC<SavingsGoalCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <GoalHeader goal={goal} IconComponent={IconComponent} t={t} />

      <GoalProgress goal={goal} formatCurrency={formatCurrency} formatDeadline={formatDeadline} t={t} />
      ...
      {goal.contributions && goal.contributions.length > 0 && (
        <ContributionHistory
          ...
          onStartEdit={startEditContribution}
          onCancelEdit={cancelEditContribution}
          onSaveEdit={saveEditContribution}
...
interface CompactSavingsGoalCardProps {
  goal: SavingsGoal;
  IconComponent: React.ComponentType<{ size: number; style?: React.CSSProperties }> | undefined;
  formatCurrency: (amount: number) => string;
...
const CompactSavingsGoalCard: React.FC<CompactSavingsGoalCardProps> = ({
  goal,
  IconComponent,
  formatCurrency,
  handleCardClick,
...
const GoalMenu: React.FC<GoalMenuProps> = ({
  goal,
  showMenu,
  onToggle,
  onClose,
...
interface ContributionHistoryProps {
  ...
  editingContribution: string | null;
  editAmount: string;
  editNote: string;
  isSubmitting: boolean;
  onStartEdit: (contribution: GoalContribution) => void;
  onCancelEdit: () => void;
              ...
              <>
                ...
                <div className="flex items-center gap-2 ml-2">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
                    <button
                      onClick={() => onStartEdit(contribution)}
                    ...
                    <button
                      onClick={() => onDelete(contribution.uuid)}
