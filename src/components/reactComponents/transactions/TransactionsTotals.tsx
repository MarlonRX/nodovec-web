      ...
      <div>
        {fixedTotals.income ? (
          <div className="flex flex-col items-center md:items-start ml-5">
            ...
            <span className="text-(--semantic-success) font-semibold text-sm md:text-base">+ ${formatAmount(fixedTotals.income)}</span>
          </div>
        ) : null}
        {fixedTotals.expense ? (
          <div className="flex flex-col items-center md:items-start mt-2 ml-5">
            ...
            <span className="text-(--semantic-error) font-semibold text-sm md:text-base">- ${formatAmount(fixedTotals.expense)}</span>
          </div>
        ) : null}
      </div>
    ) : null}
    <div className="flex flex-col items-center md:items-start p-2 md:p-3 rounded-none bg-(--bg-secondary)">
      <span className="text-(--text-tertiary) text-[8px] md:text-[9px] font-bold mb-1">{t('transactions.totalIncome')}</span>
      <span className="text-(--semantic-success) font-semibold text-sm md:text-base">+ ${formatAmount(totals.income)}</span>
    </div>
    <div className="flex flex-col items-center md:items-start p-2 md:p-3 rounded-none bg-(--bg-secondary)">
      <span className="text-(--text-tertiary) text-[8px] md:text-[9px] font-bold mb-1">{t('transactions.totalExpense')}</span>
      <span className="text-(--semantic-error) font-semibold text-sm md:text-base">- ${formatAmount(totals.expense)}</span>
    </div>
    <div className="flex flex-col items-center md:items-start p-2 md:p-3 rounded-none bg-(--bg-secondary)">
      <span className="text-(--text-tertiary) text-[8px] md:text-[9px] font-bold mb-1">{t('transactions.netBalance')}</span>
      <div className={`px-2 md:px-4 py-0.5 md:py-1 rounded-none border-2 font-semibold text-sm md:text-base transition-colors ${totals.net >= 0 ? 'text-(--semantic-success) border-(--semantic-success) bg-[rgba(46,139,87,0.1)]' : 'text-(--semantic-error) border-(--semantic-error) bg-[rgba(207,102,121,0.1)]'}`}>
