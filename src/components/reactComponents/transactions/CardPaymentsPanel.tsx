...
export const CardPaymentsPanel = ({ purchases, t }: CardPaymentsPanelProps) => (
  <div className="rounded-none overflow-hidden" style={{ border: '1px solid rgba(var(--accent-primary-rgb),0.2)', backgroundColor: 'rgba(var(--accent-primary-rgb),0.04)' }}>
    <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: 'rgba(var(--accent-primary-rgb),0.15)' }}>
      ...
      <span className="ml-auto text-xs font-bold px-2 py-0.5 rounded-none" style={{ backgroundColor: 'rgba(var(--accent-primary-rgb),0.15)', color: 'var(--accent-primary)' }}>{purchases.length} {t('transactions.cardPaymentsActive')}</span>
    </div>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 p-3">
      {purchases.map((p) => (
        <div key={p.uuid} className="flex items-center justify-between gap-3 p-3 rounded-none" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-primary)' }}>
          ...
          <span className="font-semibold text-sm shrink-0" style={{ color: 'var(--accent-primary)' }}>${formatAmount(p.installment_amount)}</span>
        </div>
      ))}
    </div>
