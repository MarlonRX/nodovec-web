...
export const TransactionsFilters = ({
  ...
  <div className="flex flex-col gap-3">
    ...
    {showFilters && (
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 p-3 rounded-none bg-(--bg-secondary) border border-(--border-primary)">
        <MySelect options={[{ value: '', label: t('transactions.allTypes') || 'All Types' }, { value: 'income', label: t('transactions.typeIncome') || 'Income' }, { value: 'expense', label: t('transactions.typeExpense') || 'Expense' }]} value={filterType} onChange={(e: any) => onFilterTypeChange(e.target.value)} className="w-full sm:w-40 h-10 text-sm font-semibold border-none bg-(--bg-primary) rounded-none" />
        <MySelect options={categoryOptions} value={filterCategory} onChange={(e: any) => onFilterCategoryChange(e.target.value)} className="w-full sm:w-48 h-10 text-sm font-semibold border-none bg-(--bg-primary) rounded-none" />
        {hasActiveFilters && (
          <button onClick={onClearFilters} className="flex items-center gap-1 px-3 py-2 text-xs font-semibold text-(--semantic-error) hover:bg-[rgba(207,102,121,0.1)] rounded-none transition-colors">
    ...
    {hasActiveFilters && (
      <div className="flex flex-wrap items-center gap-2">
        ...
        {debouncedSearch && <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-none text-xs font-semibold bg-(--accent-primary) text-(--text-inverted)"><Search className="w-3 h-3" />"{debouncedSearch}"<button onClick={() => onSearchChange('')} aria-label="Clear search" className="hover:opacity-70"><X className="w-3 h-3" /></button></span>}
        {filterType && <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-none text-xs font-semibold bg-(--bg-secondary) text-(--text-primary) border border-(--border-primary)">{filterType}<button onClick={() => onFilterTypeChange('')} aria-label="Clear type filter" className="hover:opacity-70"><X className="w-3 h-3" /></button></span>}
        {filterCategory && <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-none text-xs font-semibold bg-(--bg-secondary) text-(--text-primary) border border-(--border-primary)">{filterCategory.replace(/_/g, ' ')}<button onClick={() => onFilterCategoryChange('')} aria-label="Clear category filter" className="hover:opacity-70"><X className="w-3 h-3" /></button></span>}
      </div>
    )}
  </div>
);
